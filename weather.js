...
const goGetWeather = (lat, lon) => {
  ...
  fetch( url ).then( response => {
    ...
    response.json().then( data => {
      ...
      const { sys:{sunrise,sunset}, main:{temp}, weather, name, dt } = data;
      ...
      $(`.panel-header`).style.backgroundImage = `url("https://source.unsplash.com/random/600×400/?${name},${weather[0].main},skyline")`
...
window.onload = () => { navigator.geolocation.getCurrentPosition( 
  ( {coords: {latitude, longitude}} ) => { // good
